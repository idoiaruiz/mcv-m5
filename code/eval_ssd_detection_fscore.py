...
model = build_ssd(img_shape=input_shape,n_classes=NUM_CLASSES+1, freeze_layers_from=None)

model.load_weights(sys.argv[1], by_name = True)

bbox_util = BBoxUtility(num_classes = NUM_CLASSES, priors = None, overlap_threshold = 0.5,
...
for i,img_path in enumerate(imfiles):
  ...
  if len(img_paths)%chunk_size == 0 or i+1 == len(imfiles):
    ...
    results = bbox_util.detection_out(net_out, background_label_id=0, keep_top_k=200,
    ...
    for i,img_path in enumerate(img_paths):
        if len(results[i]) > 0:
          ...
          top_indices = [idx for idx, conf in enumerate(det_conf) if conf >= detection_threshold]
        ...
        for j in range(top_conf.shape[0]):
          if (det_label[j] == 3) and (dataset_name == 'Udacity'): continue #For udacity it loks like Truck class is always wrong 
          bx = BoundBox(len(classes))
          bx.probs[int(top_label_indices[j])-1] = 1.
          bx.w = top_xmax[j] - top_xmin[j]
          bx.h = top_ymax[j] - top_ymin[j]
          bx.x = top_xmin[j] + bx.w/2 
          bx.y = top_ymin[j] + bx.h/2 
